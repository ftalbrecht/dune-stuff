...
struct GTraits
{
  ...
  typedef Stuff::Functions::Constant<Entity, typename Grid::ctype, Grid::dimension, typename Grid::ctype, 1>
      ConstantFunction;
  typedef Stuff::Functions::Expression<Entity, typename Grid::ctype, Grid::dimension, typename Grid::ctype, 1>
      ScalarFunction;
#if HAVE_DUNE_FEM
  ...
  typedef Dune::Fem::LagrangeDiscreteFunctionSpace<ContFunctionSpace, Dune::Fem::AdaptiveLeafGridPart<Grid>,
                                                   polynomial_order> DiscreteSpace;
  typedef Fem::AdaptiveDiscreteFunction<DiscreteSpace> DiscreteFunction;
#endif
#if HAVE_DUNE_PDELAB
  typedef PDELab::QkLocalFiniteElementMap<typename Grid::LeafGridView, typename Grid::ctype, FieldType,
                                          polynomial_order> FEMap;
...
void fem(const int macro_elements, ResultMap& values)
{
  ...
  Fem::LagrangeInterpolation<typename Traits::ScalarFunction, typename Traits::DiscreteFunction>::apply(
      linear_gradient, linear_gradient_df);
  Fem::LagrangeInterpolation<typename Traits::ConstantFunction, typename Traits::DiscreteFunction>::apply(
      constant_0, constant_0_df);
  Fem::LagrangeInterpolation<typename Traits::ConstantFunction, typename Traits::DiscreteFunction>::apply(
      constant_neg1, constant_neg1_df);
